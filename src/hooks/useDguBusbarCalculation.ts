...
export const useDguBusbarCalculation = () => {
  ...
  const { loading: calculationsLoading } = useRusnCalculation(selectedGroupSlug);
  ...
  useEffect(() => {
    const fetchMaterialPrices = async () => {
      try {
        ...
        console.error('Error fetching DGU material prices:', error);
      }
    };
    fetchMaterialPrices();
  }, []);
  ...
  useEffect(() => {
    const fetchBusbarCalculation = async () => {
      try {
        ...
        console.error('Error fetching DGU busbar calculation:', error);
      }
    };
