...
function KPWrapper(props: Props) {
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined') {
      import('@react-pdf/renderer')
        ...
        .catch((error) => {
          console.error('❌ KPWrapper: Ошибка при загрузке @react-pdf/renderer:', error);
          setPdfModule(null);
          setIsLoading(false);
        });
    }
  ...
  try {
    const fileName = `${props.filename}-КП.pdf`;
    ...
    return (
      ...
      >
        {({ loading, blob }: { loading: boolean; blob: Blob | null }) => {
          // Вызываем onReady когда КП готов (не загружается и есть blob)
          ...
          return (
            <>
              <Download size={16} />
              <span>{loading ? 'Подготовка КП...' : 'Скачать КП'}</span>
            </>
          );
        }}
      </LinkComponent>
...
export default React.memo(KPWrapper, (prevProps, nextProps) => {
  ...
  return (
    ...
    prevProps.tableMarkupTotals === nextProps.tableMarkupTotals
