...
export const BusbarSystemContainer = () => {
  ...
  const {
    ...
    selectedTransformer: zeroSelectedTransformer,
  ...
  useEffect(() => {
    if (totalPrice > 0 && matchingConfig) {
      ...
      const innerParts = [`шина ${group}`, busbarSize ? `(${busbarSize})` : '', amperage].filter(Boolean);
  ...
  if (calculationsLoading) {
    return (
      <BusbarBlock title="Сборные шины" subtitle="Загрузка конфигурации…">
  ...
  return (
    <div className="space-y-5">
      <BusbarBlock
        title="Сборные шины (фазные)"
        subtitle="Расчёт по трансформатору и ячейкам РУНН"
      >
        <BusbarResults
      ...
      <BusbarBlock title="Сборные шины N (нулевые)" subtitle="Нулевая шина по конфигурации ЩО">
        <BusbarResults
